...
async function generateLandingPage(property) {
  ...
  const translations = {
    fr: {
      ...
      inProgress: 'En cours',
      secondSection: 'Seconde Section' // Ajout de la traduction pour la nouvelle section
    },
    en: {
  ...
  const filePath = path.join(__dirname, '../public/landing-pages', filename); // Corrigé le chemin
    ...
    <head>
      ...
      <style>
        * {
        ...
        /* CORRECTION: Retrait de display: flex pour permettre le scroll du contenu */
        body.has-video {
        ...
        /* Style spécifique pour le conteneur en mode vidéo */
        ...
        /* Styles de la modale et du carousel (inchangés) */
        .visit-modal {
          display: none;
          position: fixed;
          top: 0;
    ...
    <body class="${embedUrl ? 'has-video' : ''}">
      ...
      <div id="visitModal" class="visit-modal">
        <div class="visit-modal-content">
          ...
          <p id="contactName">${property.contactFirstName || ''} ${property.contactLastName || ''}</p>
          <p id="contactPhone">${property.contactPhone || ''}</p>
          <div class="contact-item">
            <button id="copyNameBtn">Copier Nom</button>
            <button id="copyPhoneBtn">Copier Téléphone</button>
          </div>
        </div>
      </div>
        ...
        fetch("https://nominatim.openstreetmap.org/search?format=json&q=" + encodeURIComponent(fullAddress))
          ...
          .then(data => {
            ...
            if (data && data.length > 0) {
              ...
              if(mapElement) mapElement.innerHTML = "${t.mapUnavailable}";
            }
          })
          .catch(err => {
            console.error(err);
            const mapElement = document.getElementById('map');
            if(mapElement) mapElement.innerHTML = "${t.mapError}";
          });
