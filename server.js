...
async function generateLandingPage(property) {
  ...
  const translations = {
    fr: {
      ...
      inProgress: 'En cours',
      discoverProperty: 'Découvrez le bien'
    },
    en: {
    ...
    pt: {
      ...
      discoverProperty: 'Descubra o imóvel'
    }
  };
...
app.post('/api/user/profile', isAuthenticated, async (req, res) => {
    try {
        ...
        let adminNotification = null; // Variable pour l'alerte admin

        // Gestion de la 2FA
        if (updates.twoFaEnabled !== undefined && updates.twoFaEnabled !== user.twoFaEnabled) {
            const oldValue = user.twoFaEnabled ? 'Activée' : 'Désactivée';
            const newValue = updates.twoFaEnabled ? 'Activée' : 'Désactivée';
            
            // Si la 2FA est activée ou désactivée, on prépare la notification
            adminNotification = {
                user: user,
                updatedField: 'Authentification à deux facteurs (2FA)',
                oldValue: oldValue,
                newValue: newValue
            };
            
            user.twoFaEnabled = updates.twoFaEnabled;
            // Si la 2FA est désactivée, on clear l'email de sécurité
            if (!updates.twoFaEnabled) {
                user.twoFaEmail = undefined;
            }
        }
        
        // Gestion de l'email de secours 2FA
        if (updates.twoFaEmail !== undefined) {
             // Vérifier si l'utilisateur a vraiment activé la 2FA
            if (!user.twoFaEnabled) {
                 return res.status(400).json({ success: false, message: "2FA non activée. Impossible d'enregistrer l'email de secours." });
            }
            
            if (updates.twoFaEmail !== user.twoFaEmail) {
                 const oldValue = user.twoFaEmail || 'Non défini';
                 const newValue = updates.twoFaEmail;
                 
                 adminNotification = {
                    user: user,
                    updatedField: 'Email de secours 2FA',
                    oldValue: oldValue,
                    newValue: newValue
                 };
                 user.twoFaEmail = updates.twoFaEmail;
            }
        }

        // On gère les autres champs si vous en aviez (firstName, lastName, etc.)
        // C'est ici que vous géreriez les autres mises à jour de profil avec des checks similaires
        
        await user.save();

        // Si une notification admin a été préparée, on l'envoie APRES la sauvegarde
        if (adminNotification) {
            await emailUtils.notifyAdminUserUpdate(
                adminNotification.user, 
                adminNotification.updatedField, 
                adminNotification.oldValue, 
                adminNotification.newValue
            );
        }

        res.json({ success: true, message: "Profil mis à jour avec succès", user: user.toObject() });
